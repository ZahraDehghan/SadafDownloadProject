 ...
 */
  ...
  //-----------------------------------------------------------new methods
  function Prepare($sqlstr,$debug=self::DEBUG_MODE) {//return statement   	
  ...
  function ExecuteStatement($parameter_array,$fetch_mode=PDO::FETCH_ASSOC,$debug=self::DEBUG_MODE) {//return statement
...
//}
  ...
  //-----------------------------------------------------------
  function Execute($sqlstr,$fetch_mode=PDO::FETCH_ASSOC,$debug=self::DEBUG_MODE) {      // for backward  compatibility
    $trans = array(self::ARABIC_YA => self::PERSIAN_YA , self::ARABIC_KAF => self::PERSIAN_KAF);
    $startTime = microtime(true);//
  ...
  //Date: 2011-Apr-28
  function ExecuteBinary($sqlstr,$debug=self::DEBUG_MODE) {
    $startTime = microtime(true);
