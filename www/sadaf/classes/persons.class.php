...
class be_persons
{
	...
	{
		$query = "select persons.* from sadaf.persons  where  persons.PersonID=? ";
...
class manage_persons
{
	...
	* @return کد داده اضافه شده	*/
	static function Add($pfname, $plname, $CardNumber)
	{
		$k=0;
		$mysql = pdodb::getInstance();
		$query = "insert into sadaf.persons (";
		$query .= " pfname";
		$query .= ", plname";
		$query .= ", CardNumber";
		$query .= ") values (";
	...
	* @return 	*/
	static function Update($UpdateRecordID, $pfname, $plname, $CardNumber)
	{
		...
		$mysql = pdodb::getInstance();
		$query = "update sadaf.persons set ";
			$query .= " pfname=? ";
			$query .= ", plname=? ";
			$query .= ", CardNumber=? ";
	...
	* @return -	*/
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sadaf.persons where PersonID=?";
		$mysql->Prepare($query);
	...
	static function SearchResultCount($pfname, $plname)
	{
		...
		$k=0;
		$ret = array();
		$query = "select count(*) as TotalCount from sadaf.persons where ";
