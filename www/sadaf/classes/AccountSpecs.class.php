...
class be_AccountSpecs
{
	...
	function LoadDataFromDatabase($RecID)
	{
		$query = "select AccountSpecs.* 
			, concat(l3.pfname, ' ', l3.plname)  as l3_plname from sadaf.AccountSpecs 
			LEFT JOIN sadaf.persons  l3 on (l3.PersonID=AccountSpecs.PersonID)  where  AccountSpecs.AccountSpecID=? ";
...
class manage_AccountSpecs
{
	...
	{
		$mysql = pdodb::getInstance();
		$query = "select count(AccountSpecID) as TotalCount from sadaf.AccountSpecs";
		if($WhereCondition!="")
		{
	...
	{
		$mysql = pdodb::getInstance();
		$query = "select max(AccountSpecID) as MaxID from sadaf.AccountSpecs";
		$res = $mysql->Execute($query);
		if($rec=$res->fetch())
		{
			return $rec["MaxID"];
	...
	* @return کد داده اضافه شده	*/
	static function Add($UserID, $UserPassword, $PersonID)
	{
		$k=0;
		$mysql = pdodb::getInstance();
		$query = "insert into sadaf.AccountSpecs (";
		$query .= " UserID";
		$query .= ", UserPassword";
		$query .= ", PersonID";
		$query .= ") values (";
	...
	* @return 	*/
	static function Update($UpdateRecordID, $UserID, $UserPassword, $PersonID)
	{
		...
		$mysql = pdodb::getInstance();
		$query = "update sadaf.AccountSpecs set ";
			$query .= " UserID=? ";
			$query .= ", UserPassword=? ";
			$query .= ", PersonID=? ";
	...
	* @return -	*/
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sadaf.AccountSpecs where AccountSpecID=?";
		$mysql->Prepare($query);
	...
	static function GetList()
	{
		...
		$query = "select AccountSpecs.AccountSpecID
			...
			, concat(l3.pfname, ' ', l3.plname)  as l3_plname  from sadaf.AccountSpecs 
			LEFT JOIN sadaf.persons  l3 on (l3.PersonID=AccountSpecs.PersonID)  ";
		$res = $mysql->Execute($query);
		$i=0;
		while($rec=$res->fetch())
		{
