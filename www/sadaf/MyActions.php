...
HTMLBegin();
// از این متد در کدهای جدید استفاده نمی شود. قرار دادن به عنوان نمونه در اینجا برای مواردی است که نیاز به 
// پشتیبانی کدهای قدیمی وجود دارد. از جهت آشنایی با این مدل از کدهای سدف.
$mysql = dbclass::getInstance();
$FromRec = 0;
if(isset($_REQUEST["FromRec"]))
	$FromRec = $_REQUEST["FromRec"];
$ItemsCount = 15;
$query = "select count(ATS) from SysAudit where UserID='".$_SESSION["UserID"]."' ";
$res = $mysql->Execute($query);
$TotalCount = 0;
if($rec = $res->FetchRow())
	$TotalCount = $rec[0];
$query = "select *, concat(g2j(ATS), ' ', substr(ATS, 12,5)) as gATS from SysAudit where UserID='".$_SESSION["UserID"]."' order by ATS DESC limit $FromRec, $ItemsCount";
$list = "";
$res = $mysql->Execute($query);
$i = 0;
