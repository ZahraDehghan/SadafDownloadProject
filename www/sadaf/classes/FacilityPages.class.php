...
class be_FacilityPages
{
	...
	{
		$query = "select FacilityPages.* from sadaf.FacilityPages  where  FacilityPages.FacilityPageID=? ";
...
class manage_FacilityPages
{
	...
	* @return کد داده اضافه شده	*/
	static function Add($FacilityID, $PageName)
	{
		$k=0;
		$mysql = pdodb::getInstance();
		$query = "insert into sadaf.FacilityPages (";
		$query .= " FacilityID";
		$query .= ", PageName";
		$query .= ") values (";
		$query .= "? , ? ";
	...
	* @return 	*/
	static function Update($UpdateRecordID, $PageName)
	{
		...
		$mysql = pdodb::getInstance();
		$query = "update sadaf.FacilityPages set ";
			$query .= " PageName=? ";
	...
	* @return -	*/
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sadaf.FacilityPages where FacilityPageID=?";
		$mysql->Prepare($query);
	...
	static function GetList($FacilityID)
	{
		...
		$query = "select FacilityPages.FacilityPageID
				...
				,FacilityPages.PageName from sadaf.FacilityPages  ";
