...
class be_UserFacilities
{
	...
	function LoadDataFromDatabase($RecID)
	{
		$query = "select UserFacilities.* 
			, l1.UserID  as l1_UserID from sadaf.UserFacilities 
			LEFT JOIN sadaf.AccountSpecs  l1 on (l1.UserID=UserFacilities.UserID)  where  UserFacilities.FacilityPageID=? ";
...
class manage_UserFacilities
{
	static function HasAccess($UserID, $FacilityID)
	{
		$k=0;
		$mysql = pdodb::getInstance();
		$query = "select * from sadaf.UserFacilities where ";
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sadaf.UserFacilities where";
		$query .= " UserID=?";
		$ValueListArray = array();
		array_push($ValueListArray, $UserID); 
		$mysql->Prepare($query);
	...
	* @return کد داده اضافه شده	*/
	static function Add($UserID, $FacilityID)
	{
		$k=0;
		$mysql = pdodb::getInstance();
		$query = "insert into sadaf.UserFacilities (";
		$query .= " UserID";
		$query .= ", FacilityID";
		$query .= ") values (";
		$query .= "? , ? ";
	...
	* @return 	*/
	static function Update($UpdateRecordID, $UserID)
	{
		...
		$mysql = pdodb::getInstance();
		$query = "update sadaf.UserFacilities set ";
			$query .= " UserID=? ";
	...
	* @return -	*/
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sadaf.UserFacilities where FacilityPageID=?";
		$mysql->Prepare($query);
	...
	static function GetList($FacilityID)
	{
		...
		$query = "select UserFacilities.FacilityPageID
			...
			, l1.UserID  as l1_UserID  from sadaf.UserFacilities 
			LEFT JOIN sadaf.AccountSpecs  l1 on (l1.UserID=UserFacilities.UserID)  ";
	...
	*/
	static function SearchResultCount($FacilityID, $UserID, $OtherConditions)
	{
		...
		$k=0;
		$ret = array();
		$query = "select count(*) as TotalCount from sadaf.UserFacilities	";
 		$cond = "FacilityID=? ";
		if($UserID!="0" && $UserID!="") 
		{
			if($cond!="") $cond .= " and ";
