...
class be_SystemFacilityGroups
{
	...
	{
		$query = "select SystemFacilityGroups.* from sadaf.SystemFacilityGroups  where  SystemFacilityGroups.GroupID=? ";
...
class manage_SystemFacilityGroups
{
	...
	* @return کد داده اضافه شده	*/
	static function Add($GroupName, $OrderNo)
	{
		$k=0;
		$mysql = pdodb::getInstance();
		$query = "insert into sadaf.SystemFacilityGroups (";
		$query .= " GroupName";
		$query .= ", OrderNo";
		$query .= ") values (";
		$query .= "? , ? ";
	...
	* @return 	*/
	static function Update($UpdateRecordID, $GroupName, $OrderNo)
	{
		...
		$mysql = pdodb::getInstance();
		$query = "update sadaf.SystemFacilityGroups set ";
			$query .= " GroupName=? ";
			$query .= ", OrderNo=? ";
		$query .= " where GroupID=?";
		$ValueListArray = array();
	...
	* @return -	*/
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sadaf.SystemFacilityGroups where GroupID=?";
		$mysql->Prepare($query);
	...
	static function GetList()
	{
		...
		$query = "select SystemFacilityGroups.GroupID
				...
				,SystemFacilityGroups.OrderNo from sadaf.SystemFacilityGroups  ";
		$res = $mysql->Execute($query);
		$i=0;
		while($rec=$res->fetch())
		{
